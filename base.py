...
from backbones import BuildActivation, BuildNormalization, constructnormcfg #BuildBackbone, 
...
class BaseSegmentor(nn.Module):
    def __init__(self, cfg, mode):
        ...
        # build classifier
        # self.classifer = Classifier(cfg['classifier']['last_inchannels'], cfg['num_classes'])
    '''forward'''
    def forward(self, x, targets=None, losses_cfg=None):
        raise NotImplementedError('not to be implemented')
    '''forward when mode = `TRAIN`'''
    def forwardtrain(self, predictions_image, predictions_mask, targets, backbone_outputs, losses_cfg, img_size, compute_loss=True):
        ...
        if hasattr(self, 'auxiliary_decoder'):
            ...
            if isinstance(self.auxiliary_decoder, nn.ModuleList):
                ...
                for idx, (out, dec) in enumerate(zip(backbone_outputs, self.auxiliary_decoder)):
                    ...
                    outputs_dict[f'pix_aux{idx+1}'] = predictions_aux
            else:
                predictions_aux = self.auxiliary_decoder(backbone_outputs[-1])
                predictions_aux = F.interpolate(predictions_aux, size=img_size, mode='bilinear', align_corners=self.align_corners)
                outputs_dict['pix_aux'] = predictions_aux
        if not compute_loss: 
            return outputs_dict
    ...
    def calculatelosses(self, predictions, targets, losses_cfg):
        ...
        for loss_name, loss_cfg in losses_cfg.items():
            if 'pix' in loss_name:
                losses_log_dict[loss_name] = self.pixel_loss(
                    prediction=predictions[loss_name],
                    target=target_seg,
                    loss_cfg=loss_cfg,
                )
            else:
                losses_log_dict[loss_name] = self.image_loss(
                    prediction=predictions[loss_name],
                    target=target_img,
                    loss_cfg=loss_cfg,
                )
    ...
    '''calculate the pixel-level loss'''
    # def pixel_loss(self, prediction, target, loss_cfg):
    #     # format prediction
    #     if prediction.dim() == 4:
    ...
    #     # return the loss
    #     return loss
    # '''calculate the image-level loss'''
    # def image_loss(self, prediction, target, loss_cfg):
    #     # format prediction
    #     prediction_iter = prediction
    #     target_iter = target
    #     # calculate the loss
